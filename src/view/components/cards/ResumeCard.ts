...
const resumeCard: render = (data: UserDetails, theme: ThemeType) => {
  ...
  if (data.kind === 'Institute') {
    ...
    Institue.degree = `
    <div class="resume-card__degree" id="resume-card__degree">${degree} from ${stream}</div>
    `
    Institue.cgpa = `
    ...
    Company.designation = `
      <div class="resume-card__designation" id="resume-card__designation">${designation}</div>
    `
    Company.skillset = `
  ...
  const html = `
      <div class="cards">
        <div class="resume-card card ${theme}-card" id="resume-card">
            ...
            ${description !== undefined ? `<div class="resume-card__description" id="resume-card__description">${description}</div>` : ''}
