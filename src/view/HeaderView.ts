...
export class HeaderView extends View<HTMLDivElement> {
  ...
  update = (data: headerData) => {
    const themeClassGenerator: () => (className: string) => classList = () => {
      return (className) => {
        ...
        const classNameGenerator: (theme: string) => string = (theme) =>
  ...
  changeTheme = (controlTheme: ControlThemeType) => {
    this.themeElement.addEventListener("click", (e: Event) => {
      let target = e.target as HTMLElement;
      target = target.closest(".nav-container__theme-btn")! as HTMLElement;
      // debugger;
      const newTheme =
